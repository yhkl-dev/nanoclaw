...
describe('getMessagesSince', () => {
  ...
  it('recovers cursor from last bot reply when lastAgentTimestamp is missing', () => {
    ...
    const msgs = getMessagesSince('group@g.us', recovered!, 'Henry', 10);
  ...
  it('returns last N messages when no bot reply and no cursor exist', () => {
    ...
    const recovered = getLastBotMessageTimestamp('fresh@g.us', 'Henry');
    expect(recovered).toBeUndefined();

    // No cursor → sinceTimestamp = '' but limit caps the result
    const msgs = getMessagesSince('fresh@g.us', '', 'Henry', 10);
    expect(msgs).toHaveLength(10);
...
describe('getNewMessages', () => {
  ...
  it('returns empty for no registered groups', () => {
    const { messages, newTimestamp } = getNewMessages([], '', 'Henry');
    expect(messages).toHaveLength(0);
