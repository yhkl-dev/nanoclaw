...
トリガーワード（デフォルト：`@Henry`）でアシスタントに話しかけます：

```
@Henry 毎朝9時に営業パイプラインの概要を送って（Obsidian vaultフォルダにアクセス可能）
@Henry 毎週金曜に過去1週間のgit履歴をレビューして、差異があればREADMEを更新して
@Henry 毎週月曜の朝8時に、Hacker NewsとTechCrunchからAI関連のニュースをまとめてブリーフィングを送って
```

メインチャネル（セルフチャット）から、グループやタスクを管理できます：
```
@Henry 全グループのスケジュールタスクを一覧表示して
@Henry 月曜のブリーフィングタスクを一時停止して
@Henry Family Chatグループに参加して
```
