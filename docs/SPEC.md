...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Henry';
...
Messages must start with the trigger pattern (default: `@Henry`):
- `@Henry what's the weather?` → ✅ Triggers Claude
- `@henry help me` → ✅ Triggers (case insensitive)
- `Hey @Henry` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Henry what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Henry what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Henry add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Henry remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Henry list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Henry remember I prefer dark mode` | Add to global memory |
...
```
User: @Henry remind me every Monday at 9am to review the weekly metrics
...
```
User: @Henry at 5pm today, send me a summary of today's emails
...
From any group:
- `@Henry list my scheduled tasks` - View tasks for this group
- `@Henry pause task [id]` - Pause a task
- `@Henry resume task [id]` - Resume a paused task
- `@Henry cancel task [id]` - Delete a task

From main channel:
- `@Henry list all tasks` - View tasks from all groups
- `@Henry schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Henry add group "Name"` in main |
